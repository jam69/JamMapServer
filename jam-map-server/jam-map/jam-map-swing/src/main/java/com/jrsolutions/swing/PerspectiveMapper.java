...
public class PerspectiveMapper implements IMapper3D {
	...
	// }
	/**
	 * Indicamos que tama�o tiene nuestra ventana. Suponemos que nuestra ventana
	 * es de tama�o (0,0)-(ancho,alto) Es fundamental que estos datos sean
	 * correctos.
	 *
	 * @param ancho
	 *            Ancho en unidades(pixels) de la ventana.
	 * @param alto
	 *            Alto en unidades(pixels) de la ventana.
	 */
	...
	/**
	 * Le mandamos una nuevas coordenadas de zoom. Las coordenadas se ajustan a
	...
	/**
	 * Devuelve la escala que estamos utilizando para dibujar. Este valor son
	 * los metros que representa cada pixel. Si supieramos el tama�o del pixel
	 * (unidad de representacion) podr�amos tener la escala real.
	 *
	...
	/**
	 * Devuelve la coordenada X real, que corresponde a la coordenada dada en
	 * las unidades de la ventana.
	 * 
	 * @param cx
	 *            Posicion horizontal del pixel.
	...
	/**
	 * Devuelve la coordenada Y real, que corresponde a la coordenada dada en
	 * las unidades de la ventana.
	 * 
	 * @param cy
	 *            Posicion Vertical del pixel.
	...
	// return
	// (int)(Math.round((r.getYMax()-cy)*alto/(r.getYMax()-r.getYMin())));
	// }
	// private int posX(double cx) {
	// return (int)Math.round((cx-r.getXMin())*ancho/(r.getXMax()-r.getXMin()));
	// }
	/**
	 * Some transformations require both coordinates (for now, we ignore X)
	 * 
	 * @param X
	 * @param Y
	 * @return
	...
	// GeneralPath res=new
	// GeneralPath(GeneralPath.WIND_EVEN_ODD,line.getNumPoints());
	...
	/**
	 * Metodo que adapta las coordenadas para que se mantenga la misma escala en
	 * horizontal y en vertical Y que este dentro del limite de zoom Original.
	 */
	...
	// @Override
	// public void drawLine(double x, double y, double x2, double y2, Graphics
	// g) {
	...
	// int SY=400;
	//
	// Point2D.Float corPix(int x0, int y0, int x1, int y1, int x2, int y2, int
	// x3,
	// int y3, int x, int y ) {
	//
	...
	//
	// float2 c2D = float2(2.0, - 2.0) * (outCoord() / imgSize) + float2(- 1.0,
	// 1.0);
	//
	...
	// z*Math.sin(k*t)+j );
	// bottomLeftA3D = new Point3D.Double( 0, centerY - z*Math.cos(k*t+k),
	// z*Math.sin(k*t+k)+j );
	// topRightA3D = new Point3D.Double( w, centerY - z*Math.cos(k*t),
	// z*Math.sin(k*t)+j );
	// bottomRightA3D = new Point3D.Double( w, centerY - z*Math.cos(k*t+k),
	// z*Math.sin(k*t+k)+j );
	// topLeftB3D = new Point3D.Double( 0, centerY - z*Math.cos(k*t-k),
	// z*Math.sin(k*t-k)+j );
	// bottomLeftB3D = new Point3D.Double( 0, centerY - z*Math.cos(k*t),
	// z*Math.sin(k*t)+j );
	// topRightB3D = new Point3D.Double( w, centerY - z*Math.cos(k*t-k),
	// z*Math.sin(k*t-k)+j );
	// bottomRightB3D = new Point3D.Double( w, centerY - z*Math.cos(k*t),
	// z*Math.sin(k*t)+j );
	//
	// g3.setPaint(shadowA);
